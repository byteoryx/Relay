...
class Clusters(Wallet):
    ...
    def process_mint(self):
        ...
        addition_hour = randint(0, 9)
        date_now = (datetime.utcnow() + timedelta(hours=addition_hour)).strftime(f"%Y-%m-%dT%H:%M:%S+0{addition_hour}:00")
        signature = self.sign_message(text=f"""clusters.xyz verification

Before interacting with certain functionality, we require a wallet signature for verification.

{date_now}""")
        self.browser.add_chains_clusters(address=self.address, domain=domain, signature=signature, date_now=date_now)
